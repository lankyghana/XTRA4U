...
@section('title', 'Edit USSD Plan - XTRA4U Admin')

@section('content')
<x-admin-layout title="Edit USSD Plan" :subtitle="$plan->name" active="ussd-plans">
    <div class="max-w-4xl">
        @if ($errors->any())
        ...
        <form action="{{ route('admin.ussd-plans.update', $plan) }}" method="POST"
              class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            @csrf
            @method('PUT')

            @include('admin.ussd-plans._form', [
                'plan' => $plan,
                'baseCode' => $baseCode,
