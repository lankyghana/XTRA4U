...
@section('title', 'Delivery Status - XTRA4U Admin')
@section('description', 'Broadcast a delivery-speed notice to all vendors')

@section('content')
<x-admin-layout title="Delivery Status" subtitle="Post a delivery-speed notice that shows on every vendor's dashboard" active="delivery-status">
    <x-slot name="actions">
        <span class="hidden sm:inline text-sm {{ $active ? 'text-green-600' : 'text-gray-400' }}">
            <svg class="inline w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
            </svg>
            {{ $active ? 'Notice is live' : 'Notice is off' }}
        </span>
    </x-slot>
    ...
    <div class="max-w-4xl"
        ...
        <form method="POST" action="{{ route('admin.settings.delivery-status.update') }}">
            ...
            {{-- Message --}}
            <div class="mt-6 bg-white border border-gray-200 rounded-xl shadow-sm p-5">
                <label for="message" class="block text-sm font-semibold text-gray-900">Notice message</label>
                <p class="mt-0.5 text-xs text-gray-500">What vendors read. Keep it short and clear.</p>
                <textarea id="message" name="message" rows="3" maxlength="500" x-model="message"
                          placeholder="e.g. Data deliveries are slower than usual today due to a network provider issue. Thank you for your patience."
                          class="mt-3 block w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-bright-blue focus:ring-brand-bright-blue text-sm">{{ old('message', $message) }}</textarea>
            </div>
