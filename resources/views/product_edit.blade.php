...
@section('title', 'Edit Product - XTRA4U')

{{--
    Visual redesign only — form action/method (PUT)/@csrf/@method, every
    field name and id, and the #external-services-config data-* attributes
    the partial script below reads are all unchanged. Same treatment as
    product_create.blade.php: wrapped in <x-vendor-layout>, grouped into
    sections, with a live preview seeded from the product's current values.
    The controller now also passes $vendor (purely additive) for the shell.
--}}
@php
    use Illuminate\Support\Str;
    $metadata = $metadata ?? [];
    $activeExternalFulfillmentProviderLabel = isset($activeExternalFulfillmentProvider) ? Str::headline($activeExternalFulfillmentProvider) : 'External Provider';
    $externalNetwork = old('external_network', data_get($metadata, "external_mappings.{$activeExternalFulfillmentProvider}.network") ?? data_get($metadata, 'external_network') ?? data_get($metadata, 'datafyhub_network'));
    $mappedExternalServiceId = old('external_service_id', data_get($metadata, "external_mappings.{$activeExternalFulfillmentProvider}.service_id") ?? '');
    $mappedExternalServiceName = old('external_service_name', data_get($metadata, "external_mappings.{$activeExternalFulfillmentProvider}.service_name") ?? '');
    $mappedExternalServiceNetwork = old('external_service_network', data_get($metadata, "external_mappings.{$activeExternalFulfillmentProvider}.network") ?? '');
    $mappedExternalServiceCapacity = old('external_service_capacity', data_get($metadata, "external_mappings.{$activeExternalFulfillmentProvider}.capacity") ?? '');
    $mappedExternalServicePrice = old('external_service_price', data_get($metadata, "external_mappings.{$activeExternalFulfillmentProvider}.price") ?? '');
    $mappedExternalServiceOfferSlug = old('external_service_offer_slug', data_get($metadata, "external_mappings.{$activeExternalFulfillmentProvider}.offer_slug") ?? '');
@endphp

@section('content')
<x-vendor-layout :vendor="$vendor" title="Edit Product" subtitle="Update bundle metadata to keep the storefront accurate" active="products">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            {{-- ============================================================
                 Form column
                 ============================================================ --}}
            <form method="POST" action="{{ route('product.update', $product->id) }}" enctype="multipart/form-data" class="lg:col-span-2 space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
                        ...
                        <div class="flex-1 flex items-center justify-between gap-3">
                            ...
                            <label for="is_active" class="inline-flex items-center gap-2 cursor-pointer flex-shrink-0">
                                <input type="hidden" name="is_active" value="0">
                                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $product->is_active) ? 'checked' : '' }} class="h-4 w-4 text-brand-violet border-gray-300 rounded focus:ring-brand-violet">
                    ...
                    <div class="p-6 space-y-4">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-violet focus:ring-brand-violet" required>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mt-1.5">If you previously selected a network that is now inactive, it remains visible until you pick a new active option.</p>
                            </div>
                            <div>
                    ...
                    <div class="p-6 space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="size" id="size" value="{{ old('size', $metadata['size'] ?? '') }}" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-violet focus:ring-brand-violet">
                            </div>
                            <div>
                                <label for="validity" class="block text-sm font-semibold text-gray-700 mb-1.5">Validity</label>
                                <input type="text" name="validity" id="validity" value="{{ old('validity', $metadata['validity'] ?? '') }}" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-violet focus:ring-brand-violet">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="tag" id="tag" value="{{ old('tag', $metadata['tag'] ?? '') }}" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-violet focus:ring-brand-violet">
                            </div>
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" name="price" id="price" value="{{ old('price', $product->price) }}" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-violet focus:ring-brand-violet pl-7" min="0" step="0.01" required>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-violet focus:ring-brand-violet">{{ old('description', $metadata['description'] ?? $product->description) }}</textarea>
                        </div>

                        <div>
                            <label for="notes" class="block text-sm font-semibold text-gray-700 mb-1.5">Customer-facing Notes</label>
                            <textarea name="notes" id="notes" rows="3" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-violet focus:ring-brand-violet">{{ old('notes', $metadata['notes'] ?? '') }}</textarea>
                        </div>

                        <div>
                            <label for="image" class="block text-sm font-semibold text-gray-700 mb-1.5">Product Image</label>
                            <div class="flex items-center gap-4">
                                <div id="image_preview_wrap" class="{{ $product->image_url ? '' : 'hidden' }} flex-shrink-0">
                                    <img id="image_preview" src="{{ $product->image_url }}" alt="{{ $product->name }}" class="w-14 h-14 rounded-lg object-cover border border-gray-200">
                                </div>
                            ...
                            <p class="mt-1.5 text-xs text-gray-500">Optional. JPG, PNG, or GIF up to 2MB. {{ $product->image_url ? 'Upload a new image to replace the current one.' : '' }}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-white rounded-xl border border-gray-200 p-5">
                        ...
                        <div class="rounded-xl border-2 border-gray-100 p-4" id="product_preview_card">
                            <div class="flex items-start gap-3">
                                <div id="preview_image_wrap" class="{{ $product->image_url ? '' : 'hidden' }} flex-shrink-0">
                                    <img id="preview_image" src="{{ $product->image_url }}" alt="" class="w-11 h-11 rounded-lg object-cover">
                                </div>
                                <div id="preview_icon_wrap" class="{{ $product->image_url ? 'hidden' : '' }} w-11 h-11 rounded-lg bg-violet-50 flex items-center justify-center flex-shrink-0">
                                    <svg class="w-5 h-5 text-brand-violet" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <p id="preview_name" class="text-sm font-semibold text-gray-900 truncate">{{ $product->name }}</p>
                                        <span id="preview_tag" class="{{ ($metadata['tag'] ?? '') ? '' : 'hidden' }} px-2 py-0.5 text-[11px] font-medium bg-green-100 text-green-700 rounded-full">{{ $metadata['tag'] ?? '' }}</span>
                                    </div>
                                    <p id="preview_size" class="text-xs text-gray-500 mt-0.5">{{ ($metadata['size'] ?? '') ?: 'Package size appears here' }}</p>
                                </div>
                            </div>
                            <div class="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                                <span id="preview_validity" class="text-xs text-green-600">{{ $metadata['validity'] ?? '' }}</span>
                                <span id="preview_price" class="text-base font-bold text-brand-violet ml-auto">GHS {{ number_format((float) $product->price, 2) }}</span>
                            </div>
                        </div>

                        <p class="text-xs text-gray-400 mt-3">Roughly how this package appears to customers on your storefront.</p>
                    </div>
                    ...
                    <div class="bg-violet-50 border border-violet-100 rounded-2xl p-5">
                        <div class="flex items-start gap-2.5">
                            ...
                            <p class="text-xs text-brand-violet-deep leading-relaxed">
                                Unchecking "Active" hides this product from your storefront without deleting it —
                                useful when you're temporarily out of stock.
                            </p>
                        </div>
                    </div>
                </div>
