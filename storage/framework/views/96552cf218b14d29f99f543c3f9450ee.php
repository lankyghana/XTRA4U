...
<script>
    window.vendorStoreData = {
        vendorId: <?php echo e($vendor->id ?? 'null'); ?>,
        categories: <?php echo json_encode($categories ?? []); ?>,
        services: <?php echo json_encode($services ?? []); ?>,
        orderRoute: '<?php echo e(route('checkout.process')); ?>'
    };
</script>
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"
    ...
    <div class="text-center my-8">
        ...
        <h1 class="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Shop with <?php echo e($vendor->name); ?></h1>
        <div class="inline-flex items-center px-4 py-2 bg-purple-100 rounded-full">
            <span class="text-sm font-medium text-purple-700">Agent: <?php echo e($vendor->name); ?> (<?php echo e($vendor->vendor_code ?? 'N/A'); ?>)</span>
        </div>
    </div>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 my-12">
    <div class="bg-gradient-to-r from-green-50 to-green-100 rounded-2xl p-8 text-center shadow-lg">
        ...
        <p class="text-gray-600 mb-6">Contact <?php echo e($vendor->name); ?> if your order takes longer than 2 hours.</p>
        <a href="https://wa.me/<?php echo e(preg_replace('/[^0-9]/', '', $vendor->phone_number)); ?>" 
           target="_blank"
           class="inline-flex items-center gap-2 px-8 py-4 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-xl transition-all duration-200 shadow-md hover:shadow-xl transform hover:scale-105">
            ...
            Contact vendor: <?php echo e($vendor->phone_number); ?>
...
function orderTracker() {
    return {
        ...
        async checkStatus() {
            ...
            try {
                const response = await fetch('<?php echo e(route("order.status.check")); ?>', {
                    method: 'POST',
                    headers: {
        ...
        startPolling() {
            ...
            this.pollingInterval = setInterval(async () => {
                ...
                try {
                    ...
                    const response = await fetch('<?php echo e(route("order.status.poll")); ?>', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\dktakyi001\Desktop\XTRA4U\resources\views/vendor_store.blade.php ENDPATH**/ ?>